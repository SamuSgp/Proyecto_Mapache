...
namespace Mapache_web
{
    public partial class Form1 : Form
    {
        ...
        private async void Form1_Load(object sender, EventArgs e)
        {
            ...
            // Carga el archivo HTML local (aseg�rate de que "map.html" est� en el proyecto)
        ...
        private void WebView21_WebMessageReceived(object sender, CoreWebView2WebMessageReceivedEventArgs e)
        {
            ...
            {
                // Obt�n usuario y contrase�a
                string user = data.username;
                string pass = data.password;

                // Llama al m�todo que valida en la BD
                bool esValido = ValidarCredenciales(user, pass);

                if (esValido)
                {
                    // Respuesta exitosa al HTML
                    webView21.CoreWebView2.ExecuteScriptAsync("mostrarMensaje('�Validaci�n exitosa!')");
                }
                else
                {
                    // Respuesta de error al HTML
                    webView21.CoreWebView2.ExecuteScriptAsync("mostrarMensaje('Usuario o contrase�a incorrectos')");
                }
            }
        }
        ...
        private bool ValidarCredenciales(string user, string pass)
        {
            ...
            // Aqu� ya no tendr�s error, porque _connectionString est� declarado en la misma clase
